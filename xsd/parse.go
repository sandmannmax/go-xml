...
import (
	...
	"github.com/sandmannmax/go-xml/internal/dependency"
	"github.com/sandmannmax/go-xml/xmltree"
)
...
func parseMinMaxRestriction(el *xmltree.Element, base Type) (time.Time, float64) {
	var date time.Time
	var decimal float64
	var rfc3339_without_timezone = strings.ReplaceAll(time.RFC3339, "Z07:00", "")
	if v, ok := base.(Builtin); ok {
		...
		if v == Date {
			...
			// format = time.RFC3339
			format = rfc3339_without_timezone
		...
		if err != nil {
			...
			d, err := time.Parse(rfc3339_without_timezone, el.Attr("", "value"))
			if err != nil {
				decimal = parseDecimal(el.Attr("", "value"))
			}
			date = d
